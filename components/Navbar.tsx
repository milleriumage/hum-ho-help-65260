...
const Navbar: React.FC<NavbarProps> = ({ navigate }) => {
  const { navbarVisibility, userSubscription, currentUser, shareShowcase, shareCreatorChat, shareAllCreators, setViewCreatorBySlug } = useCredits();
  ...
  const handleShareAllCreators = () => {
    shareAllCreators();
    setNotification('All Creators link copied!');
    setTimeout(() => setNotification(null), 2000);
  };
